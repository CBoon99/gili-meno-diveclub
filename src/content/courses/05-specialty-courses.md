---
title: 'SSI Specialty Courses'
type: 'INTERMEDIATE'
price: 1800000
duration: 2
image: 'https://images.unsplash.com/photo-1551522435-a13afa10f103?w=1600&q=80'
imageAlt: 'Diver with torch exploring coral at night'
summary: 'Pick a single skill and master it. Specialties run between 1-3 days each — combine three to make Specialty Diver.'
credential: 'SSI [Specialty] Diver — varies by course'
prerequisites:
  - 'SSI Open Water Diver, or equivalent — varies by specialty'
  - 'Some specialties require Advanced Adventurer first (Deep, Night)'
  - 'Age 12+ for most specialties'
seo:
  title: 'SSI Specialty Courses on Gili Meno — Deep, Night, Wreck, Photo & More'
  description: 'Choose your specialty. Single-day SSI courses at Meno Dive Club: Deep Diving, Night Diving, Wreck Diving, Underwater Photography, Perfect Buoyancy, and more.'
---

## One specialty at a time

Specialty courses are the fastest way to add a real, practical skill to your diving and a credential to your card. Most run over one to two days with one classroom session and two ocean dives.

Stack three full SSI specialties + 24 logged dives and you earn the **Specialty Diver** rating. Five specialties + Stress & Rescue + 50 dives gives you **Master Diver**.

### Available specialties at Meno Dive Club

We run sixteen SSI specialties, plus another half-dozen on demand with our Instructor Trainer:

- **Deep Diving** — safely down to 30-40 m on Gili Meno's deeper sites
- **Night Diving** — sleeping turtles, hunting moray eels, and bioluminescence
- **Wreck Diving** — Bounty Wreck the safe way, from outside in
- **Underwater Photography** — your camera + buoyancy + light
- **Perfect Buoyancy** — the single most useful day in any diver's logbook
- **Drift Diving** — riding currents along Meno Wall and Air Wall
- **Boat Diving** — entries, exits, and the etiquette of working a boat
- **Search & Recovery** — patterns, lift bags, the unromantic but useful skills
- **Navigation** — natural and compass, day and night
- **Fish ID & Marine Ecology** — from "lots of fish" to "I saw three species of trevally"
- **Equipment Techniques** — basic care and field troubleshooting
- **Enriched Air Nitrox (EANx)** — extend your bottom time on multi-dive days
- **Sidemount Diving** — twin tanks side-mounted, redundant air, easier on your back
- **Waves, Tides & Currents** — read sea state and time your entries safely
- **Science of Diving** — physics, physiology, and decompression theory in depth
- **React Right (CPR / first aid / O₂)** — required theory module for Stress & Rescue
- **Marine Ecology** — focused on the reefs around Gili Meno specifically
- **Diver Stress & Rescue (specialty version)** — for divers not ready for the full course
- **Free Diving Try Out** — single-day intro to apnea with a SSI Free Diving Instructor

If you want a specialty we don't run on this list, ask — we can usually arrange it with our SSI Instructor Trainer.

### Cross-certifications welcome

Already qualified through PADI, NAUI, BSAC, RAID, or another WRSTC-aligned agency? You don't need to re-do beginner courses to add SSI specialties — bring your card and a logbook and we'll cross-credit you straight in.

### Course structure

Most specialties:

- **Day 1** — One classroom session and one ocean dive
- **Day 2** — Two ocean dives, the last of which counts as your assessment

### What's included

- All instruction and SSI specialty certification
- Online manual and theory
- All equipment for the relevant specialty
- All dives, boat transfers, and logbook entries

### Price

**IDR 1,800,000** per specialty (~USD 110). Bundle three or more for a discount — ask us when you book.
